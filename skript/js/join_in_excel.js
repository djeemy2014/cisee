...
list_2_filt.forEach((itim2, index2)=>{
    list_1.forEach((itim1, index1)=>{
        ...
        if (itim1["Куда? (Служебное поле)"]==itim2['Условный номер образуемого земельного участка']){
            ...
            itim1['Наименование объектов капитального строительства (по 2 этапу)']=itim2['Наименование объектов капитального строительства']
            itim1['Этапы строительства (по 2 этапу)']=itim2['Этапы строительства']
            itim1['Наименование вида разрешенного использования земельного участка']=itim2['Наименование вида разрешенного использования земельного участка']
            itim1['Код (числовое обозначение) вида разрешенного использования земельного участка']=itim2['Код (числовое обозначение) вида разрешенного использования земельного участка']
            itim1['Категория земель']=itim2['Категория земель']
            //console.log(itim1['Вид разрешенного использования'], itim2['Вид разрешенного использования'])
        }
        if (index1==4&&itim1["Куда? (Служебное поле)"]==itim2['Условный номер образуемого земельного участка']){
            console.log( itim1['Наименование объектов капитального строительства (по 2 этапу)'],itim2['Наименование объектов капитального строительства'])}
    })
})
