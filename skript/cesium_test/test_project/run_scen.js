...
/*     function update() {
    ...
    //Загрузка тестовых зданиц
    let layer_1 = Cesium.GeoJsonDataSource.load(
        "./test_project/TRK_Kaspiy_bild_2.geojson"
    )
    .then(async function(dataSource) {
            viewer.dataSources.add(dataSource);
            let entit = dataSource.entities.values
            entit.forEach((el) => {
                
                el.polygon.heightReference=2
                el.polygon.extrudedHeightReference=el.polygon.heightReference
                //el.polygon.perPositionHeight=false
                el.polygon.height=10
                el.polygon.extrudedHeight= el.properties.extrude_h._value+10
                
                //console.log(el)
                //console.log(typeof el.properties.extrude_h._value)
                console.log(2,el.polygon.perPositionHeight)
                console.log(3,el.polygon.extrudedHeightReference)
                console.log(4,el.polygon.heightReference)
            });
 ...
 */
    ...
    viewer.camera.lookAt(
        Cesium.Cartesian3.fromDegrees(48.2123, 42.1763, 1500),
